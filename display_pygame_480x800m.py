...
class Display(DisplayBase):
	...
	def _display_loop(self):
		...
		enter_img = pygame.image.load('enter.png').convert_alpha()
		
		#create button instances
		up_button = button.Button(755, 172, up_img, 1.0)
		down_button = button.Button(755, 205, down_img, 1.0)
		enter_button = button.Button(250, 125, enter_img, 3.0)

		run = True
		while run:
					
			if up_button.draw(self.display_surface):
				self.input_event = 'UP'
			if down_button.draw(self.display_surface):
				self.input_event = 'DOWN'
			if enter_button.draw(self.display_surface):
